...
def test(file , fileResult):
    ...
    sinceReg = re.compile('^since ' , re.IGNORECASE)
    listTheTotalNumberReg = re.compile('^list the total number ' , re.IGNORECASE)
    howTallIsReg = re.compile('^how tall is ' , re.IGNORECASE)
    whatIsTheSurfaceReg = re.compile('^what is the surfacliste ' , re.IGNORECASE)
    toWhatPositionReg = re.compile('to what position ' , re.IGNORECASE)
    whichReg = re.compile('^Which ' , re.IGNORECASE)
    whichIsTheTimeStringReg = re.compile('^Which (is)* (the)*(.)* (formula|key|species|mean|username|color)')
    whichIsTheTimeReg = re.compile('^Which (is)* (the)*(.)* (time|lifetime|ISO|ID|formula|version|wavelong|number|inflation|key|species|mean|rate|STL|CPI|distance|prefix|username|complexity|provisional|redshift|charge|record|resolution|period|range|canonical|limit|electro|color|point|edition|decimal|last|heigh|AMSL|index|ILID|capacity|DC|percent|UNIT|average|CID|sectional|tension|kinship|genomic|hydrolic|KDG|pka|ISWC|package|equity|performance|altitude|indicator|surface|parity|monogram|rank|longest|emedecine|frequency|width|SCN)' , re.IGNORECASE)
    whichIsReg = re.compile('^Which is' , re.IGNORECASE) 
    whatIsTheTimeReg = re.compile('^What ((is|was|were|are)*(the)*)* (.)* (native label|time when| tag|ordinal|magnitude|magnitude scale|record held|record is held|spectral|abundance|episode in|conversion|quantity for|quantity of|episode did|surface did john|profit|lifetime of|endtime|period of time|years|incubation period|measurement|-id|short name|thermal conductivity of|clara-id|si unit|pronounciation|treatments|point in time|ZIP+|first season|start time|short-id|proportion|last name|end of time|nominal voltage|electricity voltage|vocabulary|words of jesus|by alphabet|finishing time|view|GDP|citation|z-|libris|prix|word for|IPA|routing prefix|coefficient|unit conversion|unit for|inclination|coordinate|time point|text input|revenue|term |full name|new name|official name|mathematics|Nupill|impassivity|lethal|total|period of time|minimum|diameter|Amin|elevation|altitude|index|DSBE|publication interval|female population|amount|wavelength|extreme point|density of|oxidation state|greek|prize money|base salary|numeric|current population)' , re.IGNORECASE)
    whatIsTheTimeStringReg = re.compile('^Which (is)* (the)*(.)* (formula|key|native label|mean|username|color|tag|short name|vocabulary|word of jesus|by alphabet|citation)')
    ...
    nameCombineReg = re.compile('^(what|name|tell|who|in|which|whose|onwhich)(.)name(.)' , re.IGNORECASE)
    ...
    tellMeReg = re.compile('^Tell me ' , re.IGNORECASE)
    ...
    whatAreaReg = re.compile('^(what|which)* (area|village|city|place) *(.)' , re.IGNORECASE)
    ...
    with open(file, 'r' , encoding="utf8") as rf:
        ...
        for question in questions :
            ...
            if withinReg.match(question['question']):
                ...
                category = "boolean"
                type = ["booleanameCombineRegn"]
            elif  wereReg.match(question['question']):
                category = "boolean"
                type = ["boolean"]
                
            elif listTheTotalNumberReg.match(question['question']):
                category = "literal"
                type = ["number"]
            elif howTallIsReg.match(question['question']):
                category = "literal"
                type = ["number"]
            ...
            elif nameCombineReg.match(question['question']):
                category = "resource"
                type = []
            elif sinceReg.match(question['question']):
                category = "literal"
                type = ["date"]
            elif tellMeReg.match(question['question']):
                category = "resource"
                type = []
                ...
                category = "literal"
                type = ["number"]
            elif whatIsTheSurfaceReg.match(question['question']):
                category = "literal"
                type = ["number"]
                ...
                category = "literal"
                type = ["string"]
            elif inWhatYearReg.match(question['question']):
                category = "literal"
                type = ["date"]    
                    ...
                    category = "literal"
                    type = ["date"]
            elif toWhatPositionReg.match(question['question']):
                    category = "literal"
                    type = ["string"]
            elif pronunciationAudioReg.match(question['question']):
                    category = "literal"
            ...
            #test du matching avec What    
                ...
                time = re.compile('What ((is|was)* (the)*)*(.)*(time does)', re.IGNORECASE)
                if whatYearReg.match(question['question']):
                    category ="literal"
                    type = ["date"]
                elif time.match(question['question']):
                    category ="literal"
                    type =["date"]
                elif whatWhichRateReg.match(question['question']):
                    category ="literal"
                    type = ["number"]
                elif whatIsTheTimeReg.match(question['question']):
                    category ="literal"
                    if whatIsTheTimeStringReg.match(question['question']) :
                        type = ["string"]
                    else:
                        type = ["number"]
                    ...
                    category ="resource"
                    type = []
                elif whatAreaReg.match(question['question']):
                    category ="resource"
                    type = []
            ...
            # test du matching avec Which       
                ...
                if   whichIsTheMainReg.match(question['question']):
                    ...
                    category ="literal"
                    type = ["number"]
                elif whichIsTheTimeReg.match(question['question']):
                    category ="literal"
                    if whichIsTheTimeStringReg.match(question['question']) :
                        type = ["string"]
                    else:
                        type = ["number"]
                elif whatIsNumberReg.match(question['question']):
            ...
            #test du matching avec At what    "In what year
        ...
        with open(fileResult, 'a', encoding='utf-8')as result:
            result.write(']\n')
    return fileResult            
test('smart-2022-datasets-main/smart-2022-datasets-main/AT_answer_type_prediction/wikidata/SMART2022-AT-wikidata-test.json', 'SMART2022-AT-wikidata-test-97.json')
#test('clean-2022-wikidata.json', 'result-wikidata-2022.json')
#test('smart-2022-datasets-main/smart-2022-datasets-main/AT_answer_type_prediction/dbpedia/SMART2022-AT-dbpedia-test.json', 'SMART2022-AT-dbpedia-test-97.json')
